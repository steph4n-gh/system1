...
replacement = """    scored_units = 0
    ...
    total_checks = scored_units
    matching = int(sum_unit_rates)"""

if target in typesafe_content:
    typesafe_content = typesafe_content.replace(target, replacement)
    with open(typesafe_file, 'w') as f:
        f.write(typesafe_content)
    print("Fixed evaluate_promotion_eligibility counting units with average rate")
else:
